...
starburst_request_quota_increase <- function(vcpus = 500, region = NULL) {
  ...
  if (!is.null(case_id)) {
    cat_success(sprintf("\n✓ Quota increase requested (Case ID: %s)\n", case_id))
    cat_success("✓ AWS typically approves these requests within 1-24 hours\n")
    cat_success("✓ You'll receive an email when the request is processed\n")
...
starburst_quota_status <- function(region = NULL) {
  ...
  if (quota_info$limit < 500) {
    ...
    cat_success("\n✓ Quota is sufficient for most workloads\n")
  }
