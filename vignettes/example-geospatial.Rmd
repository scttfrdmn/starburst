...
> warm worker pool — it excludes the one-time cluster startup (~2 minutes) and image
> pull. For a job this small (seconds of work), that startup dominates and running
> locally is faster overall. This example is sized for illustration; bursting pays
> off when each task runs for minutes and there are many of them. See
> `vignette("performance")` for when cloud is actually worth it.
