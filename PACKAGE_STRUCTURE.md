...
staRburst is a complete R package implementing a future backend for seamless AWS cloud bursting of parallel R workloads.
...
- `plan.starburst()` - Main function to create future plan
...
- `submit_task()` - Submit task to Fargate
...
- `starburst_status()` - Show current status
- Helper functions for AWS resources

### R/quota.R (~428 lines)
- `check_fargate_quota()` - Query quota limits
- `request_quota_increase()` - Request increase from AWS
- `starburst_request_quota_increase()` - User-facing quota request
- `starburst_quota_status()` - Display quota information
- `suggest_quota()` - Intelligent quota recommendations
- `starburst_check_quota_request()` - Monitor quota requests
...
### R/logs.R (~80 lines)
- `starburst_logs()` - View worker logs from CloudWatch
- `starburst_rebuild_environment()` - Rebuild Docker image
