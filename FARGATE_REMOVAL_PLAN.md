...
**Goal**: Simplify codebase by removing Fargate support, keeping only
EC2.

**Rationale**: - EC2 has **much faster cold start** (\<30s vs 10+ min) -
EC2 provides **instance type control** (Graviton, spot, etc.) - EC2 is
**more cost-effective** for burst workloads - Fargate adds complexity
without clear benefits for this use case

**Impact**: Breaking change - users on Fargate will need to migrate to
EC2
...
**Remove parameters**: - `launch_type` (always EC2 now)

**Add default**: - `instance_type = "c6a.large"` (default to common x86
instance)
...
**Remove logic**: - All `if (backend$launch_type == "FARGATE")`
branches - Fargate-specific configuration
...
**Remove**: - Fargate compatibility requirements - Launch type
conditionals
...
**Remove**: - Fargate pricing - Launch type conditionals
...
**Remove**: - Fargate cleanup branches - Launch type conditionals
...
**Remove from backend object**: - `launch_type` field
...
**Proceed with removal** because: 1. Package is early (v0.1.0) - fewer
existing users 2. EC2 is strictly better for this use case 3.
Simplification enables faster feature development 4. Clear migration
path exists
