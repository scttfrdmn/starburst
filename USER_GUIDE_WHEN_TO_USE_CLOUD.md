...
slower per-core than modern laptops like M4 Pro.

**BUT**: With the right instance types (c8a AMD) and spot pricing, cloud
can be both faster AND cheaper than expected.

**Cloud WINS when**: - Total workload \> 2-4 hours sequential - You’ll
run multiple batches (reuse warm workers) - Need results faster than
local parallel - Laptop needs to stay usable - Budget allows \$1-5 per
job (often less with spot!)

**Local WINS when**: - Workload \< 1 hour - One-time quick analysis -
Have powerful multi-core machine (16+ cores) - Data too sensitive for
cloud - Zero budget
...
**Key Finding**: Cloud beat M4 Pro by 2.2x with 50 workers at half the
expected cost.
...
**Bottom line**: Massive parallelism (50 workers) with fast CPUs gives
solid 2x+ speedup.
...
**staRburst lets you choose your instance type** for predictable
performance:

**Available options**: - **c8a** (AMD 8th gen): ~65-75% of M4 Pro
per-core ⭐ RECOMMENDED - **c8g** (Graviton4): ~60-70% of M4 Pro
per-core - **c7a** (AMD 7th gen): ~55-65% of M4 Pro per-core -
...
**Expected performance**: - 50 workers on c8a: ~30 minutes for 10-hour
job - Cost with spot: ~\$1.80 - **2.2x speedup vs local, \$1-2 per job**
🎯

### 2. Balance CPU per Worker

**Too few vCPUs** (1-2): - ❌ Overhead dominates - ❌ Get slowest
instances - ✓ Cheapest per worker

**Sweet spot** (2-4): - ✓ Good balance - ✓ Better instance allocation -
✓ Reasonable cost

**Too many vCPUs** (8-16): - ✓ Best instances - ✓ Most memory
bandwidth - ❌ Expensive - ❌ May be overkill

**Recommendation**: - Default: 2 vCPU, 4 GB (good for most) - CPU-heavy:
4 vCPU, 8 GB (better instances) - Memory-heavy: 2 vCPU, 8-16 GB
...
**Static assignment** (what we do now): - 50 workers, 50 tasks → 1 task
each - If one task 2x slower, total time = 2x

**Better: Dynamic assignment**: - 50 workers, 60 tasks (20%
oversubscribe) - Use first 50 completions - Cancel stragglers -
...
**EC2 c8a.xlarge** (4 vCPU - best value): - On-demand: \$0.144/hour ÷ 2
workers = \$0.072/worker - **Spot: \$0.043/hour ÷ 2 workers =
\$0.022/worker** 🏆

**EC2 c8g.xlarge** (4 vCPU Graviton4): - On-demand: \$0.152/hour ÷ 2
workers = \$0.076/worker - **Spot: \$0.046/hour ÷ 2 workers =
\$0.023/worker**
...
**Small** (1 hour equivalent, 10 workers): - 10 workers × 2 vCPU × 1
hour = \$0.81 + \$0.18 memory = **~\$1.00**

**Medium** (5 hours equivalent, 25 workers): - 25 workers × 2 vCPU × 0.2
hours (with 25x parallelism) = **~\$2.50**

**Large** (10 hours equivalent, 50 workers): - 50 workers × 2 vCPU × 0.2
hours = **~\$5.00**

**Conclusion**: Most jobs cost **\$1-5** even for many hours of
computation
...
3.  **Use warm pools for recurring jobs**: Set
    `warm_pool_timeout = 7200` (2 hours)
...
**Use c8a with spot instances**: - Multi-hour sequential jobs -
Parameter sweeps (5+ runs) - Daily/recurring production - Large-scale
parallel work (50+ tasks) - Need results in minutes, not hours

**Example**: 10-hour job - Local (M4 Pro): 60 minutes - Cloud 50
workers: 25 minutes, **\$1.38** - **Value**: Excellent!

### When Cloud Probably Wins ✓

**Good for occasional large jobs**: - 1-4 hour sequential jobs -
Occasional runs (weekly) - Medium-scale parallel (20-50 tasks) - Laptop
needs to stay usable

**Example**: 2-hour job - Local: 12 minutes - Cloud 50 workers: 8
minutes, **\$0.44** - **Value**: Good if time matters

### When Local Probably Better ⚠️

**Unless you have warm pool running**: - \< 1 hour sequential - One-time
quick analysis - Good local hardware (8+ cores)

**Example**: 30-min job - Local: 3 minutes - Cloud (warm pool): 2
minutes, \$0.07 - Cloud (cold start): 4 minutes, \$0.07 - **Value**:
Local is fine

### When Local Clearly Better ❌

**Cloud overhead not worth it**: - \< 15 minutes sequential - Tasks \<
30 seconds each - Have 16+ core workstation - Very sensitive data

**Example**: 10-min job - Local: 60 seconds - Cloud: Not worth the
setup - **Value**: Keep it local
...
- [x] Massive parallelism (scale to 100s of workers)
- [x] Fast startup (\<30s with warm pools, ~2 min cold)
- [x] Choose your CPU (c8a AMD = best price/performance)
- [x] 70% cost savings with spot instances
...
**Real examples**: - 10-hour job → 25 min for \$1.38 = **obvious win** -
1-hour job → 8 min for \$0.44 = **great value** - 15-min job → Keep it
local = **not worth overhead**

**For most researchers**: Cloud is the right choice for multi-hour
workloads. **For hobbyists**: Let it run overnight locally = \$0.

**Be honest about trade-offs. Document real performance. Help users
decide.**
